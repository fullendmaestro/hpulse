...
export function useSessions(selectedAgent: AgentListItemDto | null) {
  ...
  const apiClient = useMemo(() => (apiUrl ? new Api({ baseUrl: apiUrl }) : null), [apiUrl])
  ...
  const createSessionMutation = useMutation({
    mutationFn: async ({ state, sessionId }: CreateSessionRequest): Promise<SessionResponseDto> => {
      if (!apiClient || !selectedAgent) throw new Error('API URL and agent required')
      try {
